...
class QuizListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['top_student'] = top_student
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    ...
                    student.save()
                    if percentage < 50.0:
                        messages.warning(request, 'Mais sorte da próxima vez! Sua pontuação para o quiz %s foi %s.' % (quiz.name, percentage))
                    else:
                        messages.success(request, 'Parabéns! Você completou o quiz %s com sucesso! Você pontuou %s pontos.' % (quiz.name, percentage))
                    # return redirect('students:quiz_list')
